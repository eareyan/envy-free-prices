...
public class GreedyAllocation implements AllocationAlgoInterface{
	...
	public MarketAllocation Solve(Market market) throws CampaignCreationException{
		/* Make a copy of the campaigns array, filtering the campaigns already allocated. Sort the copied list by reward. */
		ArrayList<Campaign> campaigns = new ArrayList<Campaign>();
		for(int j=0;j<market.getNumberCampaigns();j++){
			if(market.getCampaign(j).getDemand() - market.getCampaign(j).getAllocationSoFar() > 0 & market.getCampaign(j).getReward() - market.getCampaign(j).getReserve() * (market.getCampaign(j).getDemand() - market.getCampaign(j).getAllocationSoFar()) > 0){
				campaigns.add(new Campaign(market.getCampaign(j).getDemand(),market.getCampaign(j).getReward() - market.getCampaign(j).getReserve() * (market.getCampaign(j).getDemand() - market.getCampaign(j).getAllocationSoFar()),j));
			}
		}
		Collections.sort(campaigns, this.CampaignComparator); //Sort campaigns by the given comparator
		...
		for(Campaign currentCampaign:campaigns){
			...
			for(int i=0;i<market.getNumberUsers();i++){ //Compute the accessible users to this campaign
				...
				int z_ij = (int) Math.min(market.getUser(i).getSupply() - totalAllocationFromUserSoFar[i], Math.floor(market.getCampaign(currentCampaign.getBackpointer()).getLevel()*market.getUser(i).getSupply()));
