 ...
 */
public class MultiStepEfficientAllocationILP {
	/*
	 * Market Object to be allocated
	 */
	 ...
	 */
	public MultiStepEfficientAllocationILP(Market market, int stepSize, ObjectiveFunction objectiveFunction) throws IloException, AllocationException{
		this.market = market;
	...
	/*
	 * Constructor that also takes a reserve price.
	 */
	 ...
	 */
	public ArrayList<int[][]> Solve() throws AllocationException{
		try {
			if(!this.verbose){ 
				cplex.setOut(null);
			}else{
			...
			this.cplex.setParam(IloCplex.IntParam.PopulateLim, MultiStepEfficientAllocationILP.numSolutions);
			/* Initialize indicator variables and twins. We need, per campaign, as many as the number of steps.*/
			...
			for(int j=0;j<this.market.getNumberCampaigns();j++){
				indicators.add(j, cplex.boolVarArray(Math.floorDiv(this.market.getCampaign(j).getDemand(), stepSize)));
				twinIndicators.add(j, cplex.boolVarArray(Math.floorDiv(this.market.getCampaign(j).getDemand(), stepSize)));
			}
			/* Initialize allocation variables. This is a matrix from which we are going to get the allocation.*/
			...
			for(int j=0;j<this.market.getNumberCampaigns();j++){
				int currentStep = this.stepSize;
				for(int k=0;k < indicators.get(j).length;k++){
					/* Control constraint: if the indicator y_j^k=1, then its twin must be zero. Otherwise, the twin is free. */
					this.cplex.addLe(indicators.get(j)[k],this.cplex.sum(1.0,this.cplex.prod(-1.0,twinIndicators.get(j)[k])));
					...
					/* The allocation must be at least as much as the step, where the step indicator is 1*/
					this.cplex.addGe(sumOfAlloc,indicators.get(j)[k]);
					/* If y_j^k = 1 then upper bound allocation to ensure the allocation is exactly that given by the step.*/
					this.cplex.addLe(sumOfAlloc,this.cplex.sum(indicators.get(j)[k],this.cplex.prod(Integer.MAX_VALUE,this.cplex.sum(1.0,this.cplex.prod(-1.0,indicators.get(j)[k])))));
					/* Control for the twin variable indicator*/
					for(int l=0;l<indicators.get(j).length;l++){
						/* Here we implement the constraint that if an indicator for a step of a campaign is 1, say y_j^k=1,
						 * Then, all other step indicators for that campaign must be 0, .i.e., y_j^l=0, for l\nek*/
						if(l!=k){
							this.cplex.addLe(this.cplex.prod(-1.0,indicators.get(j)[l]),this.cplex.prod(Integer.MAX_VALUE,twinIndicators.get(j)[k]));
							this.cplex.addLe(indicators.get(j)[l], this.cplex.prod(1.0*Integer.MAX_VALUE,twinIndicators.get(j)[k]));
						}
					}
					currentStep += this.stepSize;
				}
				...
				if(this.verbose){
			    	...
			    	for(int j=0;j<this.market.getNumberCampaigns();j++){
			    		...
			    		for(int i=0;i<indicatorSol.length;i++){
			    			System.out.println("I["+j+"]["+i+"] = " + indicatorSol[i]);
			    		}
			    		double[] twinIndicatorSol = this.cplex.getValues(twinIndicators.get(j));
			    		for(int i=0;i<twinIndicatorSol.length;i++){
			    			System.out.println("Itwin["+j+"]["+i+"] = " + twinIndicatorSol[i]);
			    		}
			    	}
				}
	            /*
