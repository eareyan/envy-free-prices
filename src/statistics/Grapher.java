...
public class Grapher {
  ...
  public static void gradientAscent() {
    ...
    while (currentGradientIteration < numberOfGradientIterations) {
      ...
      for (int k = 0; k < numberOfSamples; k++) {
        ...
        if (((u11 * ((u11 - reserve >= 0) ? 1 : 0)) + (u22 * ((u22 - reserve >= 0) ? 1 : 0))) > ((u12 * ((u12 - reserve >= 0) ? 1 : 0)) + (u21 * ((u21
            - reserve >= 0) ? 1 : 0)))) {
    ...
    /*
     * int samples = 1000; double sum = 0.0; reserve = 0.0; for(int k=0;k<samples;k++){ double u11 = Math.random(); double u12 = Math.random(); double u21 =
     * Math.random(); double u22 = Math.random(); sum += Math.max((u11*((u11 - reserve >= 0) ? 1 : 0)) + (u22*((u22 - reserve >= 0) ? 1 : 0)) , (u12*((u12 -
     * reserve >= 0) ? 1 : 0)) + (u21*((u21 - reserve >= 0) ? 1 : 0))); } System.out.println("Estimate = " + sum /(double) samples);
     */
  }
  ...
  public static void main(String args[]) throws UnitDemandException {
    ...
    for (int k = 0; k < numberPoints; k++) {
      ...
      expectedW += Math.max(u11 * ((u11 - reserve >= 0) ? 1 : 0) + u22 * ((u22 - reserve >= 0) ? 1 : 0), u12 * ((u12 - reserve >= 0) ? 1 : 0) + u21
          * ((u21 - reserve >= 0) ? 1 : 0));
      ...
      UnitDemandMarketOutcome matching = maxWEQAlgo.Solve();
      welfare += matching.getMarketAllocation().getWelfare();
      expectedSellerRevenue += matching.getSellerRevenue();

      /* Compute which permutation won: */
      if (u11 * ((u11 - reserve >= 0) ? 1 : 0) + u22 * ((u22 - reserve >= 0) ? 1 : 0) >= u12 * ((u12 - reserve >= 0) ? 1 : 0) + u21
          * ((u21 - reserve >= 0) ? 1 : 0)) {
        /* Identity permutation wins */
  ...
  public static void main2(String args[]) throws UnitDemandException {
    ...
    UnitDemandMarketOutcome matchingWithReserve = maxWEQAlgo.Solve();
    System.out.println("---");
    matchingWithReserve.getMarketAllocation().printAllocation();
    System.out.println("---");
    matchingWithReserve.printPrices();
    ...
    System.out.println("---");
    UnitDemandMarketOutcome newMatching = new UnitDemandMarketOutcome(matchingWithReserve.getMarketAllocation(), newPrices);
  ...
  public static void main777(String args[]) {
    ...
    while (k < 100 && R - L > 0) {
      ...
      /*
       * if (expectedRevenue(M) > expectedRevenue(L)) { L = M; M = M + (R - M) / 10.0; } else { R = M; M = L + (M - L) / 10.0; } k++;
       */
    }
  }
  ...
  /*
   * Function for plotting unit demand graphs. A graph here means a plot of the seller revenue as a function of the reserve price. Reserve prices range from 0
   * to 10, since we know that at 10 no bidder can afford an item. The procedure is the following: 1) Draw a random valuation matrix 2) "Shift" this matrix by a
   * reserve price r (see RandomMarketFactory.getValuationReserve) 3) Run MaxWeq on the shifted matrix 4) Increment all prices obtained by MaxWEQ by the reserve
   * 5) Compute the seller revenue from the matching obtained by MaxWEQ on the reserve matrix and the updated prices Repeat this process many times to get an
   * estimate for the mean seller revenue.
   */
  public static void main55(String args[]) throws IOException, UnitDemandException {
    ...
    double reserve = 0.0;
    // FileWriter fw = new FileWriter("/home/eareyanv/workspace/graphs/unitdemand-"+numberOfPoints+"-"+numberOfSamples+"-"+numUsers+"-"+numCampa+"-"+p+".csv",
    // true);
    ...
    for (int i = 0; i < numberOfPoints; i++) {
      ...
      for (int j = 0; j < numberOfSamples; j++) {
        ...
        double valueOptAllocaction = UnitDemandMarketAllocation.computeMaximumWeightMatchingValue(X).getWelfare();
  ...
  /*
   * Function for plotting multi-minded demand graphs. A graph here means a plot of the seller revenue as a function of the reserve price. Reserve prices range
   * from 0 to 10, since we know that at 10 no bidder can afford an item. The procedure is the following: 1) Draw a random market (either over or underdemanded,
   * with some coefficient b) 2) Compute the allocation that respect reserve price r 3) Get restricted envy-free prices for that reserve price. 4) Compute the
   * seller revenue as usual (allocation times prices) Repeat this process many times to get an estimate for the mean seller revenue.
   */
  public static void main3(String args[]) throws Exception {
    ...
    FileWriter fw = new FileWriter("/home/eareyanv/workspace/graphs/multiminded-" + numberOfPoints + "-" + numberOfSamples + "-" + numUsers + "-" + numCampa
        + "-" + p + "-" + overunderdemand + "-" + b + ".csv", true);
    BufferedWriter bw = new BufferedWriter(fw);
    @SuppressWarnings("resource")
    PrintWriter out = new PrintWriter(bw);
    for (int i = 0; i < numberOfPoints; i++) {
      ...
      for (int j = 0; j < numberOfSamples; j++) {
        ...
        /*
         * Compute envy-free prices RestrictedEnvyFreePricesLPWithReserve<SingleStepFunction> efp = new
         * RestrictedEnvyFreePricesLPWithReserve(allocRespectReserve); efp.setMarketClearanceConditions(false); efp.createLP(); efp.setReservePrice(reserve);
         * RestrictedEnvyFreePricesLPSolution sol = efp.Solve(); if(sol.getStatus().equals("Infeasible")){ sellerRevenue = 0.0; }else{ sellerRevenue =
         * sol.sellerRevenue(); } revenue.addValue((double) sellerRevenue / valueOptAllocaction);
         */
      }
      out.println(reserve + "," + revenue.getMean());
      out.flush();
