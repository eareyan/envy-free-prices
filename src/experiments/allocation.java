  ...
  public void runOneExperiment(int numUsers, int numCampaigns, double prob, int b, SqlDB dbLogger) throws SQLException, IloException, AllocationAlgoException, BidderCreationException, MarketAllocationException, GoodsException, AllocationException, MarketCreationException {
    if (!dbLogger.checkIfUnitDemandRowExists("allocation", numUsers, numCampaigns, prob)) {
      ...
      for (int t = 0; t < RunParameters.numTrials; t++) {
        ...
        MarketAllocation<Market<Goods, Bidder<Goods>>, Goods, Bidder<Goods>> efficient = new SingleStepWelfareMaxAllocationILP<Market<Goods, Bidder<Goods>>, Goods, Bidder<Goods>>().Solve(randomMarket);
        MarketAllocation<Market<Goods, Bidder<Goods>>, Goods, Bidder<Goods>> greedy = new GreedyMultiStepAllocation<Market<Goods, Bidder<Goods>>, Goods, Bidder<Goods>>(1, new EffectiveReachRatio()).Solve(randomMarket);
        /* Compute statistics */
