...
%M(:,3)
%M = M(M(:,3)==0.25,:)
%M = M(M(:,1)>M(:,2) & M(:,3)==1.0,:) %under-demanded
%M = M(M(:,1)>M(:,2),:) %under-demanded
%M = M(M(:,1)<M(:,2) & M(:,3)==0.25,:) %over-demanded
M = M(M(:,1)<M(:,2),:) %over-demanded
...
rowLabels = {'CK', 'EVApp' , 'MaxWE','LP'}; 
%rowLabels = {'CK', 'EVApp' , 'MaxWE'}; 
